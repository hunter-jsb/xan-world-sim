...
func (m *model) helpTree() []helpNode {
	return []helpNode{
		{title: "keys", body: []string{
			...
			"L               chronicle (slice) /      g          jump to the latest news (slice)",
			"                annals of sealed ages (deep time)",
			"n               let an age pass unwatched — seal its fate, step deep time forward",
		...
		{title: "systems", children: []helpNode{
			...
			{title: "fate — the sealed ages", body: []string{
				...
				"",
				"what survives a millennium: places, names, lines, and stories. the halls",
				"still standing fold into the next world (an old capital returns as a great",
				"river hall — the crown is sworn anew each age, and may well choose it",
				"again); every hall that fell leaves a TELL — an ash-gray h whose tooltip",
				"carries the old chronicle line, and a destination worth an expedition.",
				"",
				"the lines run deeper than the map: ruling houses keep their halls across",
				"the dawn, and deep roots steady them (a house of three ages rarely fails);",
				"a realm whose name re-forms counts its ages — 'Pabranar, 3rd of its name';",
				"dragons wake with the tempers they held, and a den the lava took stays",
				"empty; the worst feuds smolder on as embers. oaths alone are sworn anew.",
				"",
				"the ground reconciles the record: a tell the sea has drowned, the ice has",
				"taken, or the lava has buried lives on only in the annals — which L reads",
				"in deep time, age by sealed age. n lets an age pass unwatched: the same",
				"millennium runs headless and seals the same fate — history is",
				"deterministic, watched or not.",
				"",
				"sealing is choosing a future: it replaces any age previously sealed at or",
				"after that moment. each seed keeps its own chain, and it survives restarts.",
			}},
