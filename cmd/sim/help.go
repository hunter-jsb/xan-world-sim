...
func (m *model) helpTopics() []helpTopic {
	return []helpTopic{
		...
		{"simulation — politics year by year", []string{
			...
			"",
			"minor events stream through the status line; major ones (secessions, wars,",
			"sackings, foundings) pause the years with a popup — jump there or continue.",
			"L opens the chronicle: the slice's whole history, latest first, jumpable.",
		}},
		...
		{"keys", []string{
			...
			"L               the chronicle (slice)    s          expedition to cursor / abandon",
